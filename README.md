...
## Building
If any modifications are needed, then just use the `build.sh` script which will output the cursors in `dist/`.

### Dependencies
The `build.sh` script just needs `xorg-xcursorgen` and `python-cairosvg` to run.
